...
namespace LAFTestAutomationFramework.Funnel.AgentProposalForRent.Slides
{
    public class PostCodeSelection
    {
        ...
        public VerificationInfo IsNotDisplayed()
        {
            VerificationInfo info = new VerificationInfo();
            if (_postCodeSelectionSlide.VerifyDisplayStyleWith("block").Result)
                info.Result = false;
            else
                info.Result = true;

            info.Actual = _postCodeSelectionSlide.VerifyDisplayStyleWith("block").Actual;
            return info;
        }
